  ...
  return (
    ...
    >
      ...
      <div className="relative z-40 p-8">
        <motion.p
          layoutId={layout ? `description-${card.title}` : undefined}
          className="text-white text-sm md:text-base font-bold font-sans text-left"
        >
          {card.title}
        </motion.p>
        <motion.p
          layoutId={layout ? `title-${card.title}` : undefined}
          className="text-white text-xl md:text-1xl  max-w-xs text-left [text-wrap:balance] font-sans mt-2"
        >
          {card.description}
        </motion.p>
      </div>
      ...
      />
      <div className="absolute inset-x-0 bottom-0 h-0 bg-black opacity-50 transition-all duration-300 group-hover:h-full z-20 pointer-events-none" />
    </motion.button>
  </a>
