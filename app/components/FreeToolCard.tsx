...
export default function FreeToolCard({ tool }: FreeToolCardProps) {
  return (
    <Link href={tool.link}>
      <div className="group relative bg-slate-900/50 backdrop-blur-sm rounded-2xl border border-slate-800 hover:border-indigo-500/50 transition-all duration-300 overflow-hidden h-full">
        {/* Featured Badge */}
        {tool.featured && (
        ...
        <div className="p-6">
          ...
          {/* Hover Effect Border */}
          <div className="absolute inset-0 border-2 border-transparent group-hover:border-indigo-500/20 rounded-2xl transition-colors pointer-events-none"></div>
        </div>
      </div>
    </Link>
  );
